...
def parse_metadata_from_path(path):
    """
    Parse distance (cm), angle (deg), and direction label from filename.

    Supports:
      - distance-only:   distance_log_50cm.csv
      - radial distance: distance_log_R120cm_...
      - angle:           ..._A30deg_..., ..._A60deg_..., ..._A90deg...
      - direction:       suffix _n, _s, _e, _w, _ne, _nw, _se, _sw
    """
    ...
    # Distance: prefer RXXXcm, otherwise plain XXXcm
    m = re.search(r'r(\d+)\s*cm', base)
    if m is None:
        m = re.search(r'(\d+)\s*cm', base)
    if m:
        try:
    ...
    # Angle: A30deg, A45deg, A60deg, A90deg
    m = re.search(r'a(\d+)\s*deg', base)
    if m:
        try:
            angle_deg = int(m.group(1))
        except ValueError:
            angle_deg = None

    # Direction: look for _nw, _ne, _sw, _se, _n, _s, _e, _w before extension
    # check two-letter directions first to avoid matching 'n' inside 'ne'
    dir_tokens = ["nw", "ne", "sw", "se", "n", "s", "e", "w"]
    for token in dir_tokens:
        if re.search(rf"_{token}\.", base):
            direction = token
            break

    return distance_cm, angle_deg, direction
...
def analyze_file(path, fmin, fmax, t_plot):
    ...
    # parse metadata from filename (works for both distance-only and angle tests)
    distance_cm, angle_deg, direction = parse_metadata_from_path(path)
    ...
    title1 = (f"{path} | BPM_est≈{bpm_est:.2f} "
    ...
    plt.figure(figsize=(10, 4))
    title2 = (f"{path} | Aligned template (f=f_peak, phase=phi_est)\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Breathing analysis: BPM error + amplitude error with aligned template"
    )
    parser.add_argument(
        "--csv", nargs="+", required=True,
        help="One or more CSV files (distance or angle-position tests)"
    )
    parser.add_argument(
