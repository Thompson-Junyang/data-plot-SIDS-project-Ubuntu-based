...
# ---------- utility: distance from filename ----------

def parse_distance_cm_from_path(path):
    """
    Try to extract distance in cm from filename, e.g. 'distance_log_50cm.csv' -> 50.
    """
    m = re.search(r'(\d+)\s*cm', path.lower())
    if m is None:
        m = re.search(r'(\d+)cm', path.lower())
    if m:
        try:
...
def analyze_file(path, fmin, fmax, t_plot):
    ...
    # summary row for CSV
    dist_cm = parse_distance_cm_from_path(path)
    row = {
        "file": path,
        "distance_cm": dist_cm,
